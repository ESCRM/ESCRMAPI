var IDC_WindowManager = function (Document, parentObject, layout, jsVersion, cssVersion) {
    ...
    //Attach keyboard now
    var me = this;
    this.Core.Keyboard.PrimaryKeyUpFunction = function (keyCode) {
        if (me.CurrentWindow) {
            if (!keyCode)
        ...
        if (this.WindowManager.CurrentWindow.WindowState == 1) {
            if (this.WindowManager.CurrentWindowEvent.indexOf('r') == 0) {
                if (!this.WindowManager.ShowContentWhileDraggingAndResizing) {
                    this.WindowManager.CurrentWindow.SetPositionAndSize(this.WindowManager.__MovingAndResizingWindowOverlay.offsetLeft, this.WindowManager.__MovingAndResizingWindowOverlay.offsetTop, this.WindowManager.__MovingAndResizingWindowOverlay.offsetWidth - 2, this.WindowManager.__MovingAndResizingWindowOverlay.offsetHeight - this.WindowManager.CurrentWindow.TitlebarTable.offsetHeight - 2);
...
IDC_Window.prototype._Show = function (remainHidden) {
    ...
    this.ContentTable.className = 'IDCWindowContentTable';
    ...
    this.cmdClose.onmouseover = function () {
        if (this.Window.Closeable && !this.Window.IsLocked) {
            if (!this.Window.Minimizeable && !this.Window.Maximizeable)
                this.className = 'IDCWMWindowFrameTitleBarControlsButtonCloseHover' + (this.Window.IsFocused ? 'Focused' : '') + (this.Window.WindowState == 1 ? '' : 'Maximized');
            else
                this.className = 'IDCWMWindowFrameTitleBarControlsButtonCloseHover' + (this.Window.IsFocused ? 'Focused' : '') + (this.Window.WindowState == 1 ? '' : 'Maximized');
        }
    };
    this.cmdClose.onmouseout = function () {
        if (this.Window.Closeable && !this.Window.IsLocked) {
            if (!this.Window.Minimizeable && !this.Window.Maximizeable)
                this.className = 'IDCWMWindowFrameTitleBarControlsButtonClose' + (this.Window.IsFocused ? 'Focused' : '') + (this.Window.WindowState == 1 ? '' : 'Maximized');
            else
                this.className = 'IDCWMWindowFrameTitleBarControlsButtonClose' + (this.Window.IsFocused ? 'Focused' : '') + (this.Window.WindowState == 1 ? '' : 'Maximized');
        }
    };
    ...
    this.cmdMinimize.onmouseover = function () { if (this.Window.Minimizeable && !this.Window.IsLocked) this.className = 'IDCWMWindowFrameTitleBarControlsMinimizeHover' + (this.Window.IsFocused ? 'Focused' : '') + (this.Window.WindowState == 1 ? '' : 'Maximized'); };
    this.cmdMinimize.onmouseout = function () { if (this.Window.Minimizeable && !this.Window.IsLocked) this.className = 'IDCWMWindowFrameTitleBarControlsMinimize' + (this.Window.IsFocused ? 'Focused' : '') + (this.Window.WindowState == 1 ? '' : 'Maximized'); };
    this.cmdMinimize.onclick = function () { if (this.Window.Minimizeable && !this.Window.IsLocked) this.Window.Minimize(); };

    this.cmdMaximize.onmouseover = function () { if (this.Window.Maximizeable && !this.Window.IsLocked) this.className = 'IDCWMWindowFrameTitleBarControlsMaximizeHover' + (this.Window.IsFocused ? 'Focused' : '') + (this.Window.WindowState == 1 ? '' : 'Maximized'); };
    this.cmdMaximize.onmouseout = function () { if (this.Window.Maximizeable && !this.Window.IsLocked) this.className = 'IDCWMWindowFrameTitleBarControlsMaximize' + (this.Window.IsFocused ? 'Focused' : '') + (this.Window.WindowState == 1 ? '' : 'Maximized'); };
    this.cmdMaximize.onclick = function () { if (this.Window.Maximizeable && !this.Window.IsLocked) this.Window.Maximize(); };

    this.cmdRestore.onmouseover = function () { if (this.Window.Maximizeable && !this.Window.IsLocked) this.className = 'IDCWMWindowFrameTitleBarControlsRestoreHover' + (this.Window.IsFocused ? 'Focused' : '') + (this.Window.WindowState == 1 ? '' : 'Maximized'); };
    this.cmdRestore.onmouseout = function () { if (this.Window.Maximizeable && !this.Window.IsLocked) this.className = 'IDCWMWindowFrameTitleBarControlsRestore' + (this.Window.IsFocused ? 'Focused' : '') + (this.Window.WindowState == 1 ? '' : 'Maximized'); };
...
IDC_Window.prototype.GetInnerHeight = function () {
    return this.ContentIframe.offsetHeight;
    try {
...
IDC_Window.prototype.SetSize = function (w, h, ignoreMinAndMax) {
    ...
    this.ContentTable.style.width = w + 'px';
    this.TitlebarTableTdControl.style.width = '1px';

    this.TitlebarTable.style.widht = w + 'px';
    this.ContentIframe.style.width = w + 'px';
    this.ContentIframeOuterBox.style.width = w + 'px';
...
IDC_Window.prototype.Maximize = function () {
    ...
    this.TitlebarTable.style.width = w; // 
    ...
    //    if (this.isIE) {
    //        this.ContentIframe.style.width = w;
    //        this.ContentIframeOuterBox.style.height = h - this.ContentTableTdTitle.offsetHeight - 100;
    //        this.BlurredOverlay.style.height = this.ContentIframeOuterBox.style.height;
    //    }
    //    else {
    //        this.ContentIframe.style.width = '100%'; 
    //        this.ContentIframeOuterBox.style.height =  h - this.ContentTableTdTitle.offsetHeight - 100;
    //        this.BlurredOverlay.style.height =  this.ContentIframeOuterBox.style.height;
    //    }

    this.ContentIframe.style.width = w; // '100%';
    this.ContentIframe.style.height = h - this.ContentTableTdTitle.offsetHeight - this.WindowManager.Desktop.Taskbar.Control.offsetHeight;
    this.ContentIframeOuterBox.style.height = h - this.ContentTableTdTitle.offsetHeight - this.WindowManager.Desktop.Taskbar.Control.offsetHeight;
    this.BlurredOverlay.style.height = this.ContentIframeOuterBox.style.height;
    this.TitlebarTextContainer.style.width = w; // '100%';
...
IDC_Window.prototype.PickDateAndTime = function (Question, Title, Date, Time, Icon, CallBack) {
    ...
    var dtHtml = '<table border="0" cellpadding="0" cellspacing="0">' +
            ...
            '<tr>' +
            '<td><i></i></td>' +
            '<td colspan="2" align="center"><i>dd-mm-&aring;&aring;&aring;&aring;</i></td>' +
            '<td></td>' +
    ...
    var html = '<html>' +
                '<head>' +
                '<link href="' + this.WindowManager.Core.WebsiteUrl + this.WindowManager.Layout + '" type="text/css" rel="stylesheet" />' +
                '<link href="' + this.WindowManager.Core.WebsiteUrl + 'styles/' + this.WindowManager.CSSVersion + 'styles.css" type="text/css" rel="stylesheet" />' +
...
var IDC_Taskbar = function (Desktop, Document, parentObject) {
    ...
    this.MultipleWindowsControl = this.Document.createElement('DIV');
...
IDC_Taskbar.prototype.AddWindowPreviewButton = function (button) {
    var sp = this.Document.createElement('DIV');
    if (button.BoundWindow)
...
IDC_TaskbarButton.prototype.OnContextMenu = function () {
    ...
    var ctxContent = document.createElement('DIV');

    ctxBack.appendChild(ctx);
    ctx.appendChild(ctxContent);
    ...
    if (!this.Command) {
        ...
        if (wi) {
            if (wi.IsPinable) {
                ...
                var ctPinContent = document.createElement('DIV');
                ctxBack.appendChild(ctPin);
                ctPin.appendChild(ctPinContent);


                ctPin.Button = this.ChildOfGroupButton || this;
                ctPinContent.innerHTML = this.Taskbar.TextPinToTaskbar || 'Pin to taskbar';
                ctPinContent.className = 'IDCWMDesktopTaskbarContextMenuPinItem';

                ctPin.className = 'IDCWMDesktopTaskbarContextMenuItem';

                ctPin.onmouseover = function () { this.className = 'IDCWMDesktopTaskbarContextMenuItemHover'; };
                ctPin.onmouseout = function () { this.className = 'IDCWMDesktopTaskbarContextMenuItem'; };
        ...
        var ctCloseContent = document.createElement('DIV');
        ctClose.appendChild(ctCloseContent);
        ctxBack.appendChild(ctClose);

        ctClose.Button = this.ChildOfGroupButton || this;
        ctClose.ActualButton = this;

        ctCloseContent.innerHTML = this.Taskbar.TextCloseWindows || 'Close windows';
        ctCloseContent.className = 'IDCWMDesktopTaskbarContextMenuCloseItem';


        ctClose.className = 'IDCWMDesktopTaskbarContextMenuItem';

        ctClose.onmouseover = function () { this.className = 'IDCWMDesktopTaskbarContextMenuItemHover'; };
        ctClose.onmouseout = function () { this.className = 'IDCWMDesktopTaskbarContextMenuItem'; };
    ...
    ctxContent.innerHTML = ctx.Button.Name || '';
    ctx.className = 'IDCWMDesktopTaskbarContextMenuItem';
    ctxContent.className = 'IDCWMDesktopTaskbarContextMenuItemContent';
    ctxContent.style.backgroundImage = 'url(' + this.Taskbar.Desktop.WindowManager.Core.WebsiteUrl + this.ContextMenuIcon + ')';
    ...
    if (ctx.Button.Command) {
        ...
        var ctUnPinContent = document.createElement('DIV');
        if (ctxBack.lastChild.firstChild.className == 'IDCWMDesktopTaskbarContextMenuItem')
            ctxBack.insertBefore(ctUnPin, ctxBack.lastChild);
        else
            ctxBack.appendChild(ctUnPin);


        ctUnPin.appendChild(ctUnPinContent);

        ctUnPin.Button = this.ChildOfGroupButton || this;
        ctUnPinContent.innerHTML = this.Taskbar.TextUnPinToTaskbar || 'Unpin from taskbar';

        ctUnPinContent.className = 'IDCWMDesktopTaskbarContextMenuUnPinItem';
        ctUnPin.className = 'IDCWMDesktopTaskbarContextMenuItem';

        ctUnPin.onmouseover = function () { this.className = 'IDCWMDesktopTaskbarContextMenuItemHover'; };
        ctUnPin.onmouseout = function () { this.className = 'IDCWMDesktopTaskbarContextMenuItem'; };
    ...
    var ctxMPCTRL = this.Taskbar.MultipleWindowsControl; // Replaced for outer
    this.Taskbar.Desktop.WindowManager.ContextMenuManager.Show(this.Taskbar.MultipleWindowsControl, this.Taskbar.Desktop.WindowManager.Core.Mouse.X - this.Taskbar.Desktop.Control.offsetLeft, null, null, this.Taskbar.Desktop.Control.offsetHeight - this.Taskbar.Desktop.WindowManager.Core.Mouse.Y + this.Taskbar.Desktop.Control.offsetTop,
...
IDC_TaskbarButton.prototype.OnClick = function () {
    if (this.Command && this.ChildGroupedButtons.length == 0) {
        ...
        if (this.BoundWindow) {
            if (this.ChildGroupedButtons.length > 0 && this.Taskbar.AutogroupButtons) { //Outer removed
                this.Taskbar.Desktop.WindowManager.ContextMenuManager.Show(this.Taskbar.MultipleWindowsControl, null, null, null, this.Taskbar.Control.offsetHeight);
            ...
            if (this.ChildGroupedButtons.length > 1 && this.Taskbar.AutogroupButtons) { //outer removed

                var dpLeft = this.ChildGroupedButtons[this.ChildGroupedButtons.length - 1].Control.offsetLeft;
                dpLeft += (this.ChildGroupedButtons[this.ChildGroupedButtons.length - 1].Control.offsetWidth - 200) / 2;
                if (dpLeft < 0) dpLeft = 0;
                this.Taskbar.Desktop.WindowManager.ContextMenuManager.Show(this.Taskbar.MultipleWindowsControl, dpLeft, null, null, this.Taskbar.Control.offsetHeight + 5);
                this.Taskbar.FillMultipleWindowsControl(this);
            }
            else {
...
var IDC_ContextMenuItem = function (ContextMenu, Icon, Name, OnClick, ToolbarButton) {
    ...
    if (this.Icon == null && this.Name == null && this.OnClick == null) {
        ...
        if (Icon) {
            this.Control.Icon.innerHTML = '<div style="width:16px;height:16px;background-image:url(' + this.ContextMenu.WindowManager.Core.WebsiteUrl + Icon + ')">&nbsp;</div>';
...
IDC_WindowManager.prototype.PickDateAndTime = function (Question, Title, Date, Time, Icon, CallBack) {
    ...
    var html = '<html>' +
                ...
                '<link href="' + this.Core.WebsiteUrl + 'styles/' + this.CSSVersion + 'styles.css" type="text/css" rel="stylesheet" />' +
...
IDS_DragDropManager.prototype.__MakeOverlay = function () {
    ...
    this.__TransparentOverlay.oncontextmenu = function () { return false; };
